...
$manifestPath = "$here\SCOrchDev-AlertConnector.psd1"
Import-Module $manifestPath -Force

Describe -Tags 'VersionChecks' 'SCOrchDev-AlertConnector manifest' {
    ...
    It 'has a valid name in the manifest' {
        $script:manifest.Name | Should Be SCOrchDev-AlertConnector
    }
...
Describe 'Style rules' {
    $_moduleRoot = (Get-Module SCOrchDev-AlertConnector).ModuleBase

    $files = @(
        Get-ChildItem $_moduleRoot -Include *.ps1,*.psm1
    )
